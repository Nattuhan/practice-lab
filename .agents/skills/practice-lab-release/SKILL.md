...
実際の保存済み楽曲と解析結果を使う開発確認が必要な場合は、`npm run desktop:dist:mac:dev`で`PracticeLab Dev.app`を作る。通常版の設定・秘密情報・キャッシュ・ログ・自動更新は共有せず、通常版の`data`と導入済み追加ランタイムを使う。追加ランタイムはDev版から変更できない読み取り専用を維持する。通常版とDev版の同時使用は禁止し、実装済みの起動保護とデータロックを外さない。通常版の`/Applications/PracticeLab.app`は置き換えず、確認後に`node scripts/macos-verification.cjs --check`を通す。
