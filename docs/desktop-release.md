...
成果物は`desktop/dist/dev-installer/mac-arm64/PracticeLab Dev.app`です。通常版とはアプリ識別子、設定、秘密情報、キャッシュ、ログ、追加ランタイム、自動更新を分離し、`~/Library/Application Support/practice-lab/data`にある楽曲・動画・譜面・ステム・解析結果だけを共有します。開発確認版のバージョンは通常版と同じままにし、ローカル確認を理由に変更しません。
