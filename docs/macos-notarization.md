...
アップロード中の接続切断など、リモートrunが作られる前の一時的な通信失敗では、提出元アプリのSHA-256が変わっていないことを確認して同じ`sign.sh`を再実行します。再ビルドやバージョン変更は行いません。run作成後に結果が不明になった場合は、先にMornNotaryのrunと依頼ブランチを確認し、同じ成果物を重複提出しないようにします。アプリや署名処理そのものが失敗した場合は、通信失敗として再試行せずログの原因を直します。
