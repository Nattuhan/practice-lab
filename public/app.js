...
var countVoiceSamples = (sampleRate) => {
  if (!cache.has(sampleRate)) {
    const voices = {};
    for (const [number, encoded] of Object.entries(count_voice_default.samples)) {
      const bytes = Uint8Array.from(atob(encoded), (char) => char.charCodeAt(0));
      const view = new DataView(bytes.buffer);
      const length = bytes.length / 2;
      voices[number] = Float32Array.from({ length: Math.ceil(length * sampleRate / count_voice_default.sampleRate) }, (_, i3) => {
        const position = i3 * count_voice_default.sampleRate / sampleRate;
...
var alignedWav = async (buffer, beats, { tracks = [], counts = [], signal, yieldTask = () => new Promise((resolve) => setTimeout(resolve, 0)) } = {}) => {
